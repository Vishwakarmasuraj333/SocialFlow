...
export default function AllWebsitesPage() {
  ...
  const [detectingSiteId, setDetectingSiteId] = useState<string | null>(null);

  const runTechDetection = async (targetUrlOrDomain: string, isEdit = false) => {
    if (!targetUrlOrDomain || targetUrlOrDomain.trim().length < 3) return;
    setIsDetecting(true);
    try {
      const res = await fetch('/api/admin/websites/detect-tech', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: targetUrlOrDomain }),
      });
      ...
      if (data.success && data.detected) {
        if (isEdit) {
          setEditForm(prev => ({
            ...prev,
            name: prev.name && prev.name !== 'In' ? prev.name : data.detected.name,
            framework: data.detected.framework || prev.framework,
            hostingProvider: data.detected.hostingProvider || prev.hostingProvider,
            cms: data.detected.cms || prev.cms,
            notes: data.detected.notes || prev.notes,
          }));
        } else {
          setForm(prev => ({
            ...prev,
        ...
        showToast(`Real Tech Detected: ${data.detected.framework} (${data.detected.hostingProvider})`, 'success');
      }
    } catch {
      // ignore
    } finally {
  ...
  const handleQuickReDetect = async (site: any) => {
    setDetectingSiteId(site.id);
    try {
      const res = await fetch('/api/admin/websites/detect-tech', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: site.url || site.domain }),
      });
      const data = await res.json();
      if (data.success && data.detected) {
        await fetch('/api/admin/websites', {
          ...
          body: JSON.stringify({
            id: site.id,
            name: site.name && site.name !== 'In' ? site.name : data.detected.name,
            framework: data.detected.framework,
            hostingProvider: data.detected.hostingProvider,
            cms: data.detected.cms,
            notes: data.detected.notes,
          }),
        });
        showToast(`Updated ${site.domain}: ${data.detected.framework} on ${data.detected.hostingProvider}`, 'success');
        fetchWebsites();
      } else {
        showToast('Detection completed', 'info');
      }
    } catch {
      showToast('Detection request failed', 'error');
    } finally {
      setDetectingSiteId(null);
    }
  };
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800/60 mt-auto">
                  ...
                  <div className="flex items-center gap-1">
                    <Button
                      size="sm"
                      variant="ghost"
                      disabled={detectingSiteId === site.id}
                      onClick={() => handleQuickReDetect(site)}
                      className="h-8 w-8 p-0 rounded-lg text-slate-400 hover:text-amber-500 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer"
                      title="Auto-Detect Real Framework & Hosting from Live Site"
                    >
                      <Sparkles className={cn('h-3.5 w-3.5 text-amber-500', detectingSiteId === site.id && 'animate-spin text-indigo-500')} />
                    </Button>
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {form.domain && (
                  <button
                    type="button"
                    onClick={() => runTechDetection(form.domain || form.url, false)}
                    disabled={isDetecting}
                    className="text-[10px] text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1 font-medium cursor-pointer"
                  >
                    <Sparkles className={cn('w-2.5 h-2.5', isDetecting && 'animate-spin')} />
                    <span>{isDetecting ? 'Detecting...' : 'Auto-Detect Real Tech'}</span>
                  </button>
                )}
              </div>
              <input
                ...
                value={form.domain}
                onBlur={() => {
                  if (form.domain && !form.framework) {
                    runTechDetection(form.domain, false);
                  }
                }}
                onChange={(e) => {
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {editForm.domain && (
                  <button
                    type="button"
                    onClick={() => runTechDetection(editForm.domain || editForm.url, true)}
                    disabled={isDetecting}
                    className="text-[10px] text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1 font-medium cursor-pointer"
                  >
                    <Sparkles className={cn('w-2.5 h-2.5', isDetecting && 'animate-spin')} />
